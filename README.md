...
1. **API キーの管理リスク**: `OPENAI_API_KEY` や `AZURE_API_KEY` などの機密情報をローカル環境に直接置くと、他のプロセスやツールからアクセスされる可能性がある
2. **ネットワークの制御が困難**: Claude Code はファイル操作やコマンド実行などの強力な権限を持つ。ホスト環境で直接実行すると、意図しない外部通信が発生した場合に検知・制御が難しい
3. **環境の再現性**: LiteLLM のバージョン、Node.js のバージョン、Claude Code のバージョンなど、チームで揃える必要のある依存関係が多い

### Dev Container が解決すること

- **環境の隔離**: API キーはコンテナ内にのみ存在し、ホスト環境から分離される
...
├── docker-compose.yml          # コンテナオーケストレーション (dev, litellm, gitlab, gitlab-runner)
├── docker-compose.podman.yml   # Podman 用 Compose ファイル (明示的ネットワーク定義)
├── workspace/                  # コンテナ内の /workspace にマウント
