...
1. **ホストの保護** — DevContainer を構築する環境自体のセキュリティ（macOS: `mac_security_check/` / Linux: `linux_security_check/`）
2. **DevContainer 内の保護** — コンテナ内で Claude Code を安全に利用するための多層防御
3. **.env 暗号化** — SOPS + age で暗号化、秘密鍵は OS Keychain / AWS Secrets Manager に隔離（`scripts/setup-env-encryption.sh`）

**対応プラットフォーム:** macOS / Amazon Linux 2023 / Windows (WSL2)

```
ホスト (macOS / Linux)               DevContainer
┌─────────────────────┐            ┌──────────────────────────┐
│ host_security/       │            │ L7: コンテナ隔離          │
│  ・OS 自動検出       │  docker    │ L6: bypass 無効化        │
│  ・グローバル deny    │  compose   │ L5: Sandbox + Permission │
│  ・クールダウン設定   │ ─────────▶ │ L4: Post-install 監査    │
│  ・IOC 定期チェック   │            │ L3: パッケージマネージャ設定│
│  ・脅威インテリジェンス│            │ L2: Pre-install ガード    │
│                     │            │ L1: 危険コマンドブロック   │
│ cooldown_management/ │            │ L0: ファイアウォール       │
│  ・日付自動更新      │            │                          │
└─────────────────────┘            └──────────────────────────┘
```
...
| `cdn.playwright.dev`, `playwright.azureedge.net` | Playwright Chromium ダウンロード |
...
│   ├── Dockerfile                     # 開発コンテナ (Node 24 + JDK 21 + Python + Claude Code + MCP)
...
├── host_security/                     # クロスプラットフォーム共通エントリポイント
│   ├── setup.sh                       # OS 自動検出 → mac or linux に dispatch
│   ├── platform-detect.sh             # OS/distro 検出 + ポータブルラッパー関数
│   └── common-supply-chain-checks.sh  # macOS/Linux 共通セキュリティチェック
├── mac_security_check/                # ホスト Mac セキュリティツール群
│   ├── setup.sh                       # macOS 統合セットアップ（1コマンドで全設定）
...
├── cooldown_management/               # クールダウン設定セットアップ
│   └── local-cooldown-setup.sh        # ローカル PC 用クールダウン初期設定
├── scripts/                           # 開発・運用ツール
│   ├── sync-hooks.sh                  # .claude/ → workspace/.claude/ の同期
...
| MCP サーバー | Context7, Playwright, Serena（自動設定） |
...
| --- | --- | --- | --- | --- | --- |
| TypeScript / JavaScript | Node.js 24 | ESLint 9 | Prettier | TypeScript (strict) | typescript-language-server |
...
| `uv.toml` | uv | `exclude-newer = "7 days"`（相対期間）, レジストリ固定 |
| `.pip.conf` | pip | `uploaded-prior-to = "P7D"`（相対期間）, レジストリ固定 |
...
| uv v0.9.17+ | `exclude-newer = "7 days"` | 相対期間（更新不要） | `--exclude-newer "0 days"` |
| pip v26.1+ | `uploaded-prior-to = "P7D"` | ISO 8601 期間（相対、更新不要） | `--uploaded-prior-to=P0D` |
...
### バージョン pin（サプライチェーン対策）

供給網攻撃の起点になりうるため、MCP サーバーは **バージョン / commit SHA を明示的に pin** している。

| MCP | 取得元 | 現在の pin |
| --- | --- | --- |
| context7 | npm `@upstash/context7-mcp` | `4.1.0` |
| playwright | npm `@playwright/mcp` | `0.0.80` |
| serena | git `oraios/serena` | commit `949a27ef1e5fda1a6e7b561e777bcece345c6ffd`（v1.7.0 相当）|

更新運用:

`.github/workflows/mcp-version-update.yml` が **毎週月曜 00:00 UTC**（09:00 JST）に走り、
7 日クールダウンを満たす最新版を検出して自動 PR (`chore/mcp-auto-update`) を作成する。
レビュー観点（CHANGELOG / 公開日 / `/mcp` 動作確認）を確認したらマージするだけで反映完了。

手動で走らせる場合: GitHub Actions の `MCP version auto-update` を `Run workflow` で起動。

ローカルで pin 候補を確認するだけなら:

```bash
python3 .github/scripts/mcp-version-bump.py --print
```

更新対象ファイル（自動 PR が変更）:

- `.devcontainer/mcp-servers.json` の `args` 内バージョン文字列
- `.devcontainer/Dockerfile` の `ARG CONTEXT7_MCP_VERSION` / `ARG PLAYWRIGHT_MCP_VERSION`
- 本 README の pin テーブルおよび確認用コマンド例
...
npx @playwright/mcp@0.0.80 --help
...
# Claude Code 内で /mcp コマンドを実行 → 3 サーバーが表示されることを確認
```
...
WSL2 ターミナル内で Linux パスのセキュリティ設定を実行します:

```bash
# WSL2 ターミナルで実行（Windows 側ではなく Linux 側）
cd /home/<user>/claude-dev-env

# ホストセキュリティ設定（Linux として自動検出される）
...
> **注意:** WSL2 では systemd がデフォルトで無効な場合があります。
> 定期チェック（systemd タイマー）を使う場合は `/etc/wsl.conf` に以下を追加して WSL を再起動:
> ```ini
> [boot]
> systemd=true
> ```
> systemd を有効化しない場合、定期チェックのステップはスキップされます（手動実行は可能）。
