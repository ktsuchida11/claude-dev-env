# Claude Code + LiteLLM + GitHub Docker 開発環境
...
| `oauth2.googleapis.com`, `accounts.google.com`, `www.googleapis.com` | Google API (Gmail 等) |
...
| Docker 内部ネットワーク (ホストネットワーク) | litellm コンテナとの通信 |
...
claude_dev_env/
├── .devcontainer/
│   ├── Dockerfile              # 開発コンテナ (Node 20 + JDK 21 + Python + Claude Code + gh + MCP + ファイアウォール)
│   ├── devcontainer.json       # VS Code Dev Containers 設定
│   ├── init-firewall.sh        # 外部通信制限スクリプト (IPv4/IPv6 dual-stack)
│   ├── install-claude.sh       # Claude Code CLI インストールスクリプト
...
> **注意**: `workspace/` は `.gitignore` で git 管理対象外です。ワークスペース内のプロジェクトは独自の git リポジトリで管理してください。
...
| 開発ツール | git, vim, nano, jq, fzf, gh (GitHub CLI), git-delta |
| Java | Eclipse Temurin JDK 21, Maven, Gradle |
...
| ポート | 4000 |
| 役割 | `claude-litellm` エイリアスで利用。Anthropic Messages API → OpenAI API の変換プロキシ |
...
| --- | --- | --- | --- | --- | --- |
| TypeScript / JavaScript | Node.js 20 | ESLint 9 | Prettier 3.8 | TypeScript 5.8 (strict) | typescript-language-server |
| Python | Python 3.12 + uv | Ruff 0.11 | Ruff format | MyPy 1.15 | Pyright |
...
## サンプルアプリケーション (`workspace/`)

`workspace/` ディレクトリに Next.js フロントエンド + FastAPI バックエンドのサンプルアプリが含まれている。Claude Code での開発ワークフローを試すためのスターター環境として利用できる。`workspace/` は本リポジトリの git 管理対象外のため、独自のリポジトリで管理する。
...
frontend (Next.js 15 / React 19 / TypeScript)
    │
    ├── Server Action → backend API 呼び出し
    └── API Route (/api/greet) → backend API 呼び出し
            │
            ▼
backend (FastAPI / Python 3.12)
    └── GET /api/greet?name=xxx → {"message": "Hello, xxx!"}
...
| | フロントエンド | バックエンド |
| --- | --- | --- |
| フレームワーク | Next.js 15 (App Router) | FastAPI 0.115 |
| 言語 | TypeScript 5.8 (strict) | Python 3.12 |
| パッケージマネージャ | npm | uv |
| Linter | ESLint 9 (`eslint-config-next`) | Ruff (`E`, `F`, `W`, `I`, `UP`, `B`, `SIM`) |
| Formatter | Prettier (semi, double quote, 100 文字) | Ruff format (100 文字) |
| 型チェック | TypeScript (`noEmit`, strict) | MyPy (`disallow_untyped_defs`) |
| 開発サーバー | `npm run dev` (port 3001) | `uvicorn --reload` (port 8000) |
| デバッグポート | 9229 (Node.js Inspector) | 5678 (debugpy) |

### VS Code デバッグ

`workspace/.vscode/launch.json` に以下の構成が定義済み。VS Code のデバッグパネルからアタッチしてブレークポイントによるステップ実行が可能。

| 構成名 | 対象 | 接続先 |
| --- | --- | --- |
| Attach to Frontend (Next.js) | Node.js | `frontend:9230` |
| Attach to Backend (FastAPI) | debugpy | `backend:5678` |
| Attach to Frontend + Backend | 両方同時 | 上記の複合構成 |
...
2. VS Code のデバッグパネル (`Ctrl+Shift+D`) で構成を選択し、`F5` でアタッチ
3. ソースコードにブレークポイントを設定してステップ実行
...
```bash
uv run uvicorn app.main:app --reload --host 0.0.0.0 --port 8000  # 開発サーバー
uv run ruff check app/          # Linter チェック
uv run ruff format app/         # フォーマット
uv run ruff format --check app/ # フォーマットチェック（CI 用）
...
│  .npmrc / .pip.conf / .mvn-settings.xml         │
│  → ignore-scripts, レジストリ固定 等              │
├─────────────────────────────────────────────────┤
│  Layer 2: Pre-Install ガード（Hook）              │
│  supply-chain-guard.sh                          │
│  → lockfile チェック、typosquatting 検知          │
├─────────────────────────────────────────────────┤
...
| ファイル | 対象 | 主な設定 |
| --- | --- | --- |
| `.npmrc` | npm | `ignore-scripts=true`（postinstall スクリプト無効化）、レジストリ固定 |
| `.pip.conf` | pip / uv | PyPI のみに固定（`no-extra-index-url`） |
...
# セルフホスト: http://host.docker.internal:3000
CC_LANGFUSE_BASE_URL=https://cloud.langfuse.com
```

2. コンテナを再起動（`docker compose restart dev`）
