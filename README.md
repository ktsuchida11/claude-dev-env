...
1. 公開後 **7 日以上経過した版** を選定する（npm の `min-release-age=7` と整合）。日付確認は `npm view <pkg> time --json` または GitHub commit ページ
2. `.devcontainer/mcp-servers.json` の `args` 内バージョン文字列を更新
3. `.devcontainer/Dockerfile` の `ARG CONTEXT7_MCP_VERSION` / `ARG PLAYWRIGHT_MCP_VERSION` も同じ値に揃える（serena は uvx 経由のため Dockerfile 変更不要）
4. README の本表（上記）も更新
5. `docker compose build` でビルドし直し、Claude Code 内で `/mcp` を実行して 3 サーバー全部が起動することを確認
