...
<details>
<summary>Windows (WSL2) 前提条件（クリックで展開）</summary>

Windows では WSL2 上で Linux 環境を利用します（ネイティブ Windows / PowerShell には非対応）。

| ツール | 用途 | セットアップ |
|--------|------|-------------|
| WSL2 | Linux 実行環境 | `wsl --install`（管理者 PowerShell） |
| Docker Desktop for Windows | コンテナ実行 | WSL2 バックエンドを有効化 |
| Ubuntu (WSL2 ディストロ) | 推奨ディストロ | `wsl --install -d Ubuntu` |

> **重要:** 以降の全コマンドは WSL2 ターミナル内で実行してください。
> リポジトリは WSL2 ファイルシステム内（`/home/<user>/`）にクローンしてください。
> Windows ファイルシステム（`/mnt/c/...`）はパフォーマンスが大幅に低下します。
>
> 詳細は [README.md の Windows セクション](README.md#windows-環境での利用) を参照。
