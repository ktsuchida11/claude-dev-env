...
| 4 | `com.zui.security-check.plist` | 週次チェックの launchd 設定 | `~/Library/LaunchAgents/` |
| 5 | `com.zui.threat-intel-update.plist` | 日次IOC更新の launchd 設定 | `~/Library/LaunchAgents/` |
...
sed "s|__HOME__|$HOME|g" com.zui.threat-intel-update.plist \
  > ~/Library/LaunchAgents/com.zui.threat-intel-update.plist

sed "s|__HOME__|$HOME|g" com.zui.security-check.plist \
  > ~/Library/LaunchAgents/com.zui.security-check.plist

# launchd に登録
launchctl load ~/Library/LaunchAgents/com.zui.threat-intel-update.plist
launchctl load ~/Library/LaunchAgents/com.zui.security-check.plist

# 動作確認（手動トリガー）
launchctl start com.zui.threat-intel-update
launchctl start com.zui.security-check
```
...
| 時刻 | 処理 | launchd Label |
|------|------|---------------|
| 毎日 07:00 | 脅威情報取得 → IOC DB 更新 | `com.zui.threat-intel-update` |
| 毎週月曜 09:00 | Mac フルスキャン (17項目) | `com.zui.security-check` |
...
```bash
KNOWN_PREFIXES="com.apple.|com.google.|...|com.zui.|your.app."
```
...
# 再登録
launchctl unload ~/Library/LaunchAgents/com.zui.security-check.plist
launchctl load ~/Library/LaunchAgents/com.zui.security-check.plist
```
...
_このドキュメントは Zui合同会社 のセキュリティ運用ガイドとして作成されました。_
_最終更新: 2026年4月9日_
